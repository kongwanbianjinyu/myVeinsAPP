...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        sendBeaconEnd = new cMessage("beacon stop",SEND_BEACON_END);
        CHABoardcast = new cMessage("start to boardcast CHA",CHA_BOARDCAST);
        sendReqJoinEvt = new cMessage("start to send ReqJoin event",SEND_REQJOIN_EVT);
        sendACKJoinEvt = new cMessage("start to send ACKJoin event",SEND_ACKJOIN_EVT);
        ACKTimeExpire = new cMessage("ACKTime expire, don't receive ACKJoin",ACK_TIME_EXPIRE);
        ...
        if (par("avoidBeaconSynchronization").boolValue() == true) {
            ...
            if (sendBeacons) {
                ...
                scheduleAt(300.0+collectTime,sendBeaconEnd);
            }
        }
    }
}
...
void DemoBaseApplLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        ...
        bsm->setSenderAddress(myId);//sender's MAC address
        bsm->setPsid(-1);
...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg)
{
    ...
    else if(CHAMessage *CHA = dynamic_cast<CHAMessage*>(wsm)){
        receivedCHAs++;
        onCHA(CHA);
    }
